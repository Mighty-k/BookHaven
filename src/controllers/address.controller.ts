...
export class AddressController {
  ...
  async updateAddress(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const addressIndex = (user as any).getAddressIndex(addressId);
      ...
      Object.keys(updates).forEach((key) => {
        if (key !== "_id") {
          (addressToUpdate as any)[key] = updates[key];
        }
      });
  ...
  async deleteAddress(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      // Use the model method to find address by ID
      const addressToDelete = (user as any).getAddressById(addressId);
      ...
      user.addresses = user.addresses.filter(
        (addr) =>
          !((addr as any)._id && (addr as any)._id.toString() === addressId),
      );
  ...
  async setDefaultAddress(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const addressExists = (user as any).getAddressById(addressId);
