...
class CartController {
    ...
    async getCart(req, res, next) {
        try {
            ...
            const user = await user_model_1.User.findById(userId).select("cart");
            res.json({
                success: true,
                data: user?.cart || [],
            });
    ...
    async addToCart(req, res, next) {
        try {
            ...
            const existingItemIndex = user.cart.findIndex((item) => item.bookId === bookId);
    ...
    async mergeCart(req, res, next) {
        try {
            ...
            for (const guestItem of guestCart) {
                const existingItem = user.cart.find((item) => item.bookId === guestItem.bookId);
                if (existingItem) {
